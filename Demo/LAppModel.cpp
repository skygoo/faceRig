    ...
    , _userTimeSeconds(0.0f)
{
    ...
    _idParamMouseOpenY = CubismFramework::GetIdManager()->GetId(ParamMouthOpenY);
...
{
    LAppFaceDetect* app_face_detect = LAppFaceDetect::GetInstance();
    app_face_detect->faceDetect();
...
//    }
    _model->SetParameterValue(_idParamEyeLOpen, app_face_detect->left_eye);
    _model->SetParameterValue(_idParamEyeROpen, app_face_detect->right_eye);
    _model->SetParameterValue(_idParamBrowLY, app_face_detect->eyebrow_left);
    _model->SetParameterValue(_idParamBrowRY, app_face_detect->eyebrow_right);
    _model->SetParameterValue(_idParamMouseOpenY, app_face_detect->mouth_open);
...
//    }
    ...
    //ドラッグによる変化
    //ドラッグによる顔の向きの調整
    _model->AddParameterValue(_idParamAngleX, app_face_detect->x_rotate * 30); // -30から30の値を加える
    _model->AddParameterValue(_idParamAngleY, app_face_detect->y_rotate * 30);
    _model->AddParameterValue(_idParamAngleZ, app_face_detect->z_rotate * -30);

    //ドラッグによる体の向きの調整
    _model->AddParameterValue(_idParamBodyAngleX, app_face_detect->x_rotate * 10); // -10から10の値を加える

    //ドラッグによる目の向きの調整
    _model->AddParameterValue(_idParamEyeBallX, app_face_detect->x_rotate); // -1から1の値を加える
    _model->AddParameterValue(_idParamEyeBallY, app_face_detect->y_rotate);
